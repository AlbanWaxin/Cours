...
\section{Exercice 6}
\subsection*{Version Langages}
\defis{Rappel: Langage rationnel}
Un langage rationnel est un langage qui dénote d'une expriossion rationnelle\\
Soit $\exists \alpha \in$ Ratexp tel que $[\alpha] = L$
\defie

Soit $L \in Rat$\\
Soit $\alpha$ tel que $[\alpha] = L$\\
$\alpha$ est de la forme : $\alpha := \emptyset|\varepsilon|a|\alpha\alpha|\alpha+\alpha| \alpha^*$\\
on a alors 
\begin{itemize}
    \item $\emptyset^R = \emptyset$
    \item $\varepsilon^R = \varepsilon$
    \item $a^R = a$
    \item $(\alpha + \beta)^R = \alpha^R +\beta^R$
    \item $(\alpha \beta)^R = \beta^R \alpha^R$
    \item $(\alpha^*)^R = (\alpha^R)^*$
\end{itemize}
Donc le langage est bien rationnel
...
\chapter{Automates d'états finis}
\section{Exercice 9 et 10}

\begin{enumerate}
    \item Littéralement la question b de l'exercice 4
    \item la meme chose que la question a de l'exercice 4
